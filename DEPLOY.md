...
1. **NS у регистратора.** Домен зарегистрирован в **RU-CENTER (nic.ru)**. 05.08 там выставлены
   `ns1.beget.com` и `ns2.beget.com`, реестр `.gallery` изменение ещё не разнёс.
   Альтернатива без делегирования — вернуть NS руцентровские и прописать A-записи
   `@` и `www` на `5.101.153.53`; тогда почту на домене придётся вести в nic.ru.

   ⚠️ **Про IP аккаунта — важно.** Соседние домены сделаны именно A-записью, без делегирования:
   stargift.ru и gaspdiamonds.com живут на DNS RU-CENTER и указывают на **45.130.40.111**.
   Но аккаунт сейчас обслуживает **5.101.153.53** (`m2.chase.beget.com`): по нему отдаются
   все три домена, включая relictum.gallery, а старый `45.130.40.111` на relictum.gallery
   отвечает «домен не прилинкован». Проверка — подстановкой Host:
   `curl -s -o /dev/null -w "%{http_code}" -H "Host: relictum.gallery" http://<IP>/`.
   Отсюда два вывода: новым доменам прописывать только `5.101.153.53`, а у поддержки Beget
   стоит выяснить, не выводится ли `45.130.40.111` из эксплуатации — иначе stargift.ru
   и gaspdiamonds.com однажды отвалятся без предупреждения.
