...
- **Кэш Beget — 30 дней на статику.** Перезаписали картинку под тем же именем — инкрементьте
  cache-buster `?v=N` во всех рендерах (сейчас `v=11`), иначе у посетителей останется старая.
- **Данные версионируются автоматически.** `catalog.js` и `promo-data.js` отдаются с кэшем на
  неделю, поэтому сборщик дописывает скриптам `?v=<хэш содержимого>` — правка каталога доезжает
  до посетителя сразу, руками ничего делать не нужно.
- **Формат кадра = формат контейнера.** Карточка героя жёстко 4:5 с `object-fit:contain`:
  кадр 1208×1500 вместо 1200×1500 уже даёт видимую полосу. Генератор отдаёт «4:5» неточно —
  после генерации кадрировать до точного формата.
...
- **Страницы объектов рисуются на клиенте** (`/objects/exhibit.html?id=…`), поэтому title и OG
  у них общие. Статичные шер-страницы удалены — при нужде per-object OG собирать заново.
