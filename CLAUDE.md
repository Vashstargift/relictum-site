...
## Архитектура данных
- **`shared/catalog.js`** — единый источник каталога: `window.RELICTUM_CATALOG` (**79 объектов** на 14.08.2026 — проверено загрузкой файла; было 38 на 05.08. **ТОЛЬКО реальная коллекция**; выдуманные концепт-карточки удалены по решению владельца, сгенерированные «под концепт» фото заменены на реальные). Поля: id (`R–XXXX`, тире ДЛИННОЕ «–»), slug, world/worldLabel, category, period, region, priceValue (null = «по запросу»), price, name, latin, meta, img (имя файла в shared/img без .jpg), age, location, **found** (дата находки — отдельный пункт паспорта «Дата находки», ВСЕГДА с годом и «г.»: `"1967 г."`; заполнять только когда дата известна, в `location` её не смешивать), size, **mount** (оформление — пункт паспорта «Оформление»), description, href (промо-страница). catalog.html/object.html рендерят карточки и паспорта автоматически.
- **`16_product_promos/promo-data.js`** — контент промо (**79 записей** на 14.08.2026, ключ = `id` вида `R–XXXX`, НЕ slug): hook, heroKicker, story[], era{}, alive{title,text,video,poster}, interior{video,img,text}, gallery[], heroVideo, **spin** (360°-видео).
  Покрытие ассетами на 14.08.2026 (проверено скриптом): `spin` — 37 из 79, `interior` — 61, `alive` — 36, `gallery` ≥ 4 кадров — 58. Медиатека `shared/img` — 665 файлов.
...
Статичные шер-страницы `16_product_promos/<name>.html` удалены (см. коммит «девять страниц-визиток собираются из шаблона») — все экспонаты открываются через `exhibit.html?id=<slug>`; per-object OG пока нет.
...
- **Промо-страницы** (exhibit.html, единый шаблон): видео-хиро → паспорт → история эпохи → **«Объект в 360°»** → «Встретить живым» (видео существа) → «В вашем доме» → галерея ракурсов → корзина/CTA.
...
- object.html/exhibit.html — клиентский рендер: title/OG статичные (фолбэк); per-object OG сейчас нет (статичные шер-страницы удалены).
